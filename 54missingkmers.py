...
for defline, seq in mcb185.read_fasta(sys.argv[1]):
    ...
    prot_seq = mcb185.translate(seq)
    rev_prot_seq = mcb185.translate(seq)
    
    kcount = {}
    for k in range(1, math.ceil(((len(prot_seq) / 2) + 1))):
        for i in range(0, len(seq) - k + 1):
            kmer = prot_seq[i:i+k]
            if kmer not in kcount: kcount[kmer] = 0
            kcount[kmer] += 1
        for i in range(1, len(rev_prot_seq) - k + 1):
            kmer = rev_prot_seq[i:i+k]
            if kmer not in kcount: kcount[kmer] = 0
            kcount[kmer] += 1
        
        missing = {}
        found = 0
        for nts in itertools.product('ACDEFGHIKLMNPQRSTVWY', repeat = k):
            kmer = ''.join(nts)
            if kmer in kcount: 
                found += 1
            else: 
